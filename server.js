...
function upvoteComment(url, request) {
  ...
  let exisComm = database.comments[id],
    response = {};

  if (username && exisComm && database.users[username]) {
    if (exisComm.upvotedBy.indexOf(username) === -1) {
      exisComm.upvotedBy.push(username);
    }
    exisComm.downvotedBy.splice(exisComm.downvotedBy.indexOf(username), 1);

    response.body = { comment: exisComm };
    response.status = 200;
  } else {
    response.status = 400;
  }
...
function downvoteComment(url, request) {
  ...
  if (username && exisComm && database.users[username]) {
    if (exisComm.downvotedBy.indexOf(username) === -1) {
      exisComm.downvotedBy.push(username);
    }
    exisComm.upvotedBy.splice(exisComm.upvotedBy.indexOf(username), 1);
    response.body = { comment: exisComm };
    response.status = 200;
  } else {
    response.status = 400;
...
function updateComment(url, request) {
  const requestComment = request.body && request.body.comment,
    id = Number(url.split("/").filter(segment => segment)[1]),
    exisComm = database.comments[id];
  response = {};
  ...
  if (!id || !requestComment || requestComment.body === "") {
    ...
    exisComm.body = requestComment.body;
    response.body = { comment: database.comments[requestComment.id] };
    response.status = 200;
  }
...
function deleteComment(url, request) {
  const id = Number(url.split("/").filter(segment => segment)[1]),
    existComm = database.comments[id],
    response = {};

  if (existComm) {
    //remove comment from user
    const userComments = database.users[existComm.username].commentIds;
    userComments.splice(userComments.indexOf(id), 1);

    //remove comment from article
    const articleComments = database.articles[existComm.articleId].commentIds;
...
function loadDatabase() {
  return (file = YAML.load("./database.yaml"));
}

function saveDatabase() {
  fs.writeFile("./database.yaml", YAML.stringify(database), function(err) {
    if (err) {
      return false;
    }
    return true;
